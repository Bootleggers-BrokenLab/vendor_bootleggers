...
func init() {
	android.RegisterModuleType("bootleggers_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = bootleggersExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = bootleggersExpandVariables(ctx, dep_file)
	...
	cmd := bootleggersExpandVariables(ctx, String(g.properties.Cmd))
